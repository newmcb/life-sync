    ...
    <header className="fixed top-0 left-0 w-full h-16 z-50 flex items-center background-blur shadow bg-white">
      <div className="container mx-auto flex items-center justify-between px-4 max-w-4xl">
        <div className="flex items-center">
          <div className="text-gray-500 text-xl font-bold">Life Sync
            ...
            {headerData.map((v)=>{
              return (
                <div key={v.key} onClick={() => {
                  handleRoute(v.to)
                }} className="text-gray-700 hover:text-black hover:font-bold cursor-pointer">{v.ko}</div>
              )
            })}
          </nav>
        </div>
        <div className="flex items-center space-x-6">
          <button className="text-gray-700 hover:text-black hover:font-bold">Search</button>
          <button className="text-gray-700 hover:text-black hover:font-bold">Login</button>
        </div>
      </div>
    </header>
  );
